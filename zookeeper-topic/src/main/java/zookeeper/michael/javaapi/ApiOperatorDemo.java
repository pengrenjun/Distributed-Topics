...
public class ApiOperatorDemo implements Watcher{
    ...
    public static void main(String[] args) throws IOException, InterruptedException, KeeperException {
        ...
        ACL acl=new ACL(ZooDefs.Perms.ALL,new Id("ip","10.0.99.197"));
        List<ACL> acls=new ArrayList<>();
        acls.add(acl);
//        zookeeper.create("/authTest","111".getBytes(),acls,CreateMode.PERSISTENT);
        zookeeper.getData("/node1",true,new Stat());
        System.out.println(zookeeper.getState());

        //创建节点
        String result=zookeeper.create("/nodeTest","123".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
        ...
        if(watchedEvent.getState()==Event.KeeperState.SyncConnected){
            if(Event.EventType.None==watchedEvent.getType()&&null==watchedEvent.getPath()){
                ...
                System.out.println(watchedEvent.getState()+"-->"+watchedEvent.getType());
                    ...
                    //watch监听是一次性的，如果后续操作继续监听的话，需要传true
                try {
